...
from watchingdog.events import FileSystemEvenHandler
...
class FileMovementHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        name, extension = os.path.splittext(event.scr_path)

        time.sleep(1)

        for key, value in dir_tree.iteams():
            time.sleep(1)

            if extension in value:

                file_name = os.path.basenane(event.src_path)
